...
7. **🛡️ Bảo mật thông tin định danh tối đa (.env & Git Safe)**:
   * Không còn hardcode trực tiếp API Key hay Client ID trong mã nguồn để loại bỏ nguy cơ lộ lọt thông tin.
   * Hệ thống hỗ trợ nạp cấu hình qua file môi trường `.env` (`VITE_SPREADSHEET_ID`, `VITE_API_KEY`, `VITE_CLIENT_ID`) hoặc cấu hình trực tiếp trên giao diện web an toàn.
   * Tệp tin `.env` được tự động bỏ qua khi đẩy mã nguồn lên GitHub nhờ cập nhật `.gitignore` chuẩn hóa.
...
* **Cách B: Dành cho lập trình viên (Sử dụng File `.env` cục bộ)**:
  1. Copy file [frontend/.env.example](file:///c:/Users/dangn/.gemini/antigravity/scratch/personal_webapp/frontend/.env.example) thành `.env` hoặc `.env.local` ở thư mục `frontend/`.
...
Chúc bạn có những trải nghiệm tuyệt vời và quản lý hiệu quả cuộc sống cùng **Personal Dashboard v1.2.0**! 🚀
